...
FPATH = './data/gold_senti_newtag.tsv'
NEGPATH = './data/gold_senti_neg.tsv'
...
def load_external_labels(session, candidate_class, annotator_name='gold', setup='pos'):
    if setup == 'pos':
        gold_labels = pd.read_csv(FPATH, sep="\t")
    elif setup == 'neg':
        gold_labels = pd.read_csv(NEGPATH, sep="\t")
        
    for index, row in gold_labels.iterrows():    

        # We check if the label already exists, in case this cell was already executed
        context_stable_ids = "~~".join([row['adjective']])
        ...
        context_stable_ids = "~~".join([row['word']])
        query = session.query(StableLabel).filter(StableLabel.context_stable_ids == context_stable_ids)
        query = query.filter(StableLabel.annotator_name == annotator_name)
        if query.count() == 0:
            session.add(StableLabel(
